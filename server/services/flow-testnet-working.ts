 ...
 * Using simplified transaction format that works with Flow REST API
 */
export async function createRealAgentAuthorization(
  ...
  try {
    ...
    // Get account info with proper sequence number
    ...
    log(`Flow account sequence number: ${sequenceNumber}`, 'flow-debug');

    // Create simple authorization transaction script
    const script = `
    ...
    // Simplified transaction payload - using exact Flow REST API format
    const transaction = {
    ...
    log(`Submitting Flow transaction with sequence ${sequenceNumber}`, 'flow-debug');
